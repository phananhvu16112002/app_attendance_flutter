...
//                   return AlertDialog(
//                     title: Text('Alert'),
//                     content: Text('Face is not alive! Please try again.'),
//                     actions: <Widget>[
//                       TextButton(
//                         onPressed: () {
...
//   //     return true;
//   //   } else if (isSmiling && checkFace && areEyesOpen) {
//   //     return true;
//   //   } else {
//   //     return false;
//   //   }
...
//   // bool isFakeFace(Face face) {
//   //   // Kiểm tra kích thước của khuôn mặt
//   //   if (face.boundingBox.width < MIN_FACE_WIDTH ||
//   //       face.boundingBox.height < MIN_FACE_HEIGHT) {
//   //     return true;
//   //   }

//   //   // Kiểm tra các đặc điểm cụ thể của khuôn mặt, ví dụ: tỉ lệ chiều dài và chiều rộng của mắt
//   //   int leftEyeWidth = face.landmarks[FaceLandmarkType.leftEye]!.position.x -
//   //       face.landmarks[FaceLandmarkType.leftEye]!.position.y;
//   //   int rightEyeWidth = face.landmarks[FaceLandmarkType.rightEye]!.position.x -
//   //       face.landmarks[FaceLandmarkType.rightEye]!.position.y;
//   //   int eyeWidthDifference = (leftEyeWidth - rightEyeWidth).abs();

//   //   if (eyeWidthDifference > MAX_EYE_WIDTH_DIFFERENCE) {
//   //     return true;
//   //   }
  ...
  Future<void> onCapture(File? image) async {
    if (image != null) {
      if (isFaceAlive && !isSpoof) {
        ...
        await SecureStorage().writeSecureData('image', _imageTest.path);
        await SecureStorage().writeSecureData('imageOffline', _imageTest.path);
        if (mounted) {
          if (widget.page.contains('AttendanceNormal')) {
            Navigator.pushAndRemoveUntil(
              context,
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text('Alert'),
              content:
                  Text('Face is not alive or is spoofed! Please try again.'),
              actions: <Widget>[
                TextButton(
                  onPressed: () {
                    Navigator.of(context).pop();
